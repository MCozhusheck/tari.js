...
import { providers } from "tari.js/src";
import type { WalletDaemonParameters } from "tari.js/src/providers/wallet_daemon";

const { TariPermissions, WalletDaemonTariProvider } = providers.walletDaemon;

export interface TariWalletDaemonConnectDialog {
  open: boolean;
  onClose: () => void;
  signalingServerUrl: string;
  permissions: typeof TariPermissions;
  optionalPermissions: typeof TariPermissions;
}
...
export function TariWalletDaemonConnectDialog(props: TariWalletDaemonConnectDialog) {
  ...
  useEffect(() => {
    if (open) {
      ...
      WalletDaemonTariProvider.build(params).then((provider) => {
